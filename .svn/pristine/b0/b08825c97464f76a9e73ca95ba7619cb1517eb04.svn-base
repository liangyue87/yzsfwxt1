...
public class CalculateHours {
     ...
     * @return String 结束时间
     */
    public Date getEndTime(String beginTime, float calculateHours) {
        //对输入的字符串形式的时间进行转换
        Date t1 = stringToDate(beginTime);//真实开始时间
        Date t2 = stringToDate(beginTime);
        ...
        if (calculateHours < hoursPerDay) {
            ...
            //中午休息时间段
                ...
                sourceMillisecond = (Float.valueOf((hh1 - h2) * 60).intValue());
                //下午上班时间设定
                t2.setHours(pbh);
                t2.setMinutes(pbm);
                t2 = getPreTime(t2, sourceMillisecond);
            ...
            //下午上班时间段
                ...
                sourceMillisecond = (Float.valueOf((hh1 - h2) * 60).intValue()) + Float.valueOf((h3 - h2) * 60).intValue();
                t2.setHours(pbh);
                t2.setMinutes(pbm);
                t2 = getPreTime(t2, sourceMillisecond);
                float hh2 = t2.getHours() + (float) t2.getMinutes() / 60;
...
//                t2.setDate(t2.getDay()+1);
            ...
            t2 = getEndTime(beginTime1, calculateHours);
        }
        return t2;
    }
     ...
     *
     */
    private Date getWorkDay(Date t){
        //非周六日也休息的日期
