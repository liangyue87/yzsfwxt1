...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            initView() {
                let html = "";
                html = '<div class="window rank_two line_three " id="C0180">发起用户结算</div>' +
                    '<div class="window rank_three line_three " id="C0018">编制用户结算</div>' +
                    '<div class="window rank_four line_three " id="C0019">用户结算审核</div>';
                  /* '<div class="window rank_five line_three " id="C0020">审核</div>'*/

                document.getElementById("flowDemo").innerHTML = html;
                this.plumb.addEndpoint('C0180', {uuid: 'C0180', anchors: ['Right'], isSource: true});
                this.plumb.addEndpoint('C0018', {uuid: 'C0018', anchors: ['Left'], isSource: true});
                this.plumb.addEndpoint('C0018', {uuid: 'C00181', anchors: ['Right'], isSource: true});
                this.plumb.connect({uuids: ['C0180', 'C0018'],detachable: false});
                this.plumb.addEndpoint('C0019', {uuid: 'C0019', anchors: ['Left'], isSource: true});
                this.plumb.connect({uuids: ['C00181', 'C0019'],detachable: false});
              /*  this.plumb.addEndpoint('C0019', {uuid: 'C00191', anchors: ['Right'], isSource: true});*/
               /* this.plumb.addEndpoint('C0020', {uuid: 'C0020', anchors: ['Left'], isSource: true});
                this.plumb.connect({uuids: ['C00191', 'C0020'],detachable: false});*/
                        ...
                        for(let i =0;i<list.length;i++){
                            ...
                            if(obj.BJZT == '1'){ //任务已经完成 判断红黄绿灯情况
                                ...
                                $(id).click(function () {//
                                    $(id).addClass("pointer");
                                    if (id == "#C0019") {
                                        _this.$router.push({path: '/Ckyhsh', query: {xmid: xm,lctbh:'jssh'}});
                                    }
                                });
                                if(obj.SFHONGD == '1'){ //红灯
