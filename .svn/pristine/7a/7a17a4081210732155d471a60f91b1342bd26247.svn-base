...
<template>
	<section>
		<el-row>
				...
				<el-form  :model="filters" >
					...
					<el-form-item>
						...
						<el-col :span="3" style="text-align: center">
							<el-button type="primary" v-on:click="getKcrwFk">查询</el-button>
						</el-col>
					</el-form-item>
				</el-form>
			</el-col>
			<el-col :span="24" class="toolbar">
			<!--列表-->
			<el-table :data="row" highlight-current-row v-loading="listLoading" border style="width: 100%;">
				...
				<el-table-column property="slsj" label="受理时间" width="150" align="center">
				</el-table-column>
				<el-table-column property="xmdz" label="工程地址"  min-width="180" >
				</el-table-column>
				<el-table-column label="操作" width="250" >
					<template slot-scope="scope">
						<el-button size="small" type="primary" @click="toKxfk(scope.$index, scope.row)">反馈勘查结果</el-button>
					</template>
				</el-table-column>
			</el-table>
			</el-col>
...
<script>
	...
	import { getKcrwFk } from '../../../api/xkylx';
	export default {
		data() {
			return {
				filters: {
		...
		methods: {
			//获取勘查反馈任务
			getKcrwFk() {
				let params = Object.assign({ kcbm: '1',page: this.page, size: this.size }, this.filters);
				this.listLoading = true;
				getKcrwFk(params).then((res) => {
			...
			//跳转至反馈勘查任务界面
			toKxfk: function (index, row) {

				//setXmidToSession(params).then((res) =>{
				this.$router.push({path: '/Rwfksjy', query: {xmid : row.id}})
				//});
