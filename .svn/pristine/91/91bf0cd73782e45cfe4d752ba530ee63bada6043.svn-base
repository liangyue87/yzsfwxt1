                ...
                <el-table :data="row"  v-loading="listLoading" style="width: 100%;" height="500px" :row-style="rowClass" :header-cell-style="{fontSize:'15px',background:'#f5f7fa' }">
                    ...
                    <el-table-column label="操作" width="150" align="center" fixed="right">
                        <template slot-scope="scope">
                            <label title="发起验收" @click="handleEdit(scope.$index, scope.row)">
                                <i class="iconfont icon-faqi_" style="cursor:pointer;padding: 2px;color: #66b1ff"></i></label>
                            <!--<el-button size="small" type="primary" @click="handleEdit(scope.$index, scope.row)"><i class="iconfont icon-tiaocha"></i>发起验收</el-button>-->
                            <!--<el-button size="small" type="primary" @click="handleEdit(scope.$index, scope.row)">发起验收</el-button>-->
                        </template>
                    </el-table-column>
                </el-table>
            </el-col>
...
<script>
    ...
    import{getSqzhysLsit}from'../../api/zhys';
    export default {
        data(){
            return{
                filters:{
        ...
        methods:{
            ...
            getHsjglr_vue(flag) {
                ...
                getSqzhysLsit(params).then((res) => {//getSqzhysLsit
            ...
            handleEdit: function (index, row) {
                this.$router.push({path: '/Sqzhyslbsj',query: {xmid:row.id ,sqlx:row.sqlx,sqbh:row.sqbh}});
