...
<script>
	...
	import { getKcrw , getXckcZpTask } from '../../../api/xkylx';
	export default {
		data() {
			return {
				filters: {
		...
		methods: {
			//获取勘查任务
			getXckcZpTask() {
				let params = Object.assign({ type: '1',page: this.page, size: this .size }, this.filters);
			...
			toZpry: function (index, row) {
				...
				this.$router.push({path: '/Rwzpsjy', query: {xmid : row.id}})
				//});
